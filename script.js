function setCookie(cookieName, cookieValue, minutes) {
    ...
    document.cookie = `${cookieName}=${cookieValue}${expires}; path=/`; //set cookie with expiry time
}
...
function checkUser() {
    ...
    let previousPoint = getCookieValue("point");  // gets user previous point
    if (username != "") {
...
function checkUserGuess(userGuess, mySecretNumber) {
    while (userGuess != 'q') {
        if (parseFloat(userGuess) == mySecretNumber) {
            moveToNextStage()   // moves user to next stage
            break
...
function moveToNextStage() {
    previousPoint = getCookieValue('point')  // gets user previous point
    setCookie('point', parseInt(previousPoint) + 5); // updates user's point
    newPoint = parseInt(getCookieValue('point')); // gets user's updated point 
    max = max + 1;  // increases the range 
